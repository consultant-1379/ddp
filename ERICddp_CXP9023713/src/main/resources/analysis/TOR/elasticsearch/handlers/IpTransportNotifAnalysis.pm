package IpTransportNotifAnalysis;
...
sub init($$$$) {
    ...
    if ( exists $r_incr->{'IpTransportNotifAnalysis'} ) {
        $self->{'r_notifStats'} = $r_incr->{'IpTransportNotifAnalysis'}->{'r_notifStats'};
    }
    else {
        $self->{'r_notifStats'} = {};
    }
...
sub handle($$$$$$$) {
    ...
    if ( $::DEBUG > 9 ) { print "IpTransportNotifAnalysis::handle got message from $host $program : $message\n"; }
    ...
    if ( $program eq 'JBOSS' ) {
        if( $message =~ /NOTIFICATION_RECEIVER_HANDLER.*NOTIFICATION_STATS\s*(\d{4}-\d{2}-\d{2}).(\d{2}:\d{2}:\d{2})\s*-\s*(\d{4}-\d{2}-\d{2}).(\d{2}:\d{2}:\d{2})/ ) {
            ...
            if ( $::DEBUG > 7 ) { print "IpTransportNotifAnalysis::handle notification_message : $message\n"; }
...
sub done($$$) {
    ...
    if ( $::DEBUG > 7 ) { print Dumper("IpTransportNotifAnalysis::done r_notifStats", $self->{'r_notifStats'}); }
    ...
    if ( $::DEBUG > 9 ) { print Dumper("IpTransportNotifAnalysis::done serverIdMap ", $serverIdMap); }

    # Store 'Notification Analysis Details'
    my $bcpFileNotifStats = "$tmpDir/enm_mscmipnotification_logs.bcp";
    ...
    dbDo( $dbh, sprintf("DELETE FROM enm_mscmipnotification_logs WHERE siteid = %d AND endtime BETWEEN '%s 00:00:00' AND '%s 23:59:59'",
                        $self->{'siteId'}, $self->{'date'}, $self->{'date'}) )
        or die "Failed to delete old data from enm_mscmipnotification_logs";

    dbDo( $dbh, "LOAD DATA LOCAL INFILE '$bcpFileNotifStats' INTO TABLE enm_mscmipnotification_logs" )
        or die "Failed to load new data from '$bcpFileNotifStats' file to 'enm_mscmipnotification_logs' table";

    unlink($bcpFileNotifStats);

    $r_incr->{'IpTransportNotifAnalysis'} = {
