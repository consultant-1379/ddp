...
public class Parser {
    ...
    public int parse(final String thisLine,final String first,final StringTokenizer matcher) {
        ...
        if (headerFound) {
            ...
            if ("proc-sz".equals(lastHeader)) {
                /** added by eeicjon */
                if (sarPROCSZ == null) {
                    sarPROCSZ = new procszSar(mysar);
                    if ( mysar.myUI != null ) {
                        sarPROCSZ.addtotree(mysar.graphtree);
                    }
                    mysar.pdfList.put("SolarisprocszSar", sarPROCSZ);
                    sarPROCSZ.setGraphLink("SolarisprocszSar");
                }
                /** end eeicjon */
                return 1;
            } else
        ...
        try {
            ...
            if ("proc-sz".equals(statType)) {
                /** added by eeicjon */
                // split value  "/"
                String[] vals = lastHeader.split("/");
                val1 = new Float(vals[0]);
                sarPROCSZ.add(now, val1);
                /** end eeicjon */
                return 1;
            } else
    ...
    /** Added by eeicjon **/
    private procszSar sarPROCSZ = null;
}
