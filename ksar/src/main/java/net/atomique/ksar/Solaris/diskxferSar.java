...
public class diskxferSar extends AllGraph {
    ...
    public void add(final Second now,  final Float val1Init,final Float val2Init,final Float val3Init) {
        ...
        if (tmpInt == null) {
            /* EEICJON: remove the 512 multiplication factor - we want the blocks / second value */
            t_xfer.add(now, val1Init.floatValue(), do_notify());
        } else {
            /* EEICJON: remove the 512 multiplication factor - we want the blocks / second value */
            t_xfer.update(now, new Float((val1Init.floatValue()) + (tmpInt.floatValue())));
        }
    ...
    public JFreeChart makegraph(final Second g_start, final Second g_end) {
        ...
        NumberFormat decimalformat1 = new IEEE1541Number(1, 512);
